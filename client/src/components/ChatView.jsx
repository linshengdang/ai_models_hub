...
export default function ChatView({ 
  ...
  // 1. Unselected State: Show huge selector card in center of screen
  if (!providerId || !modelId || showDropdownSelector) {
    return (
      <div className="chat-view">
        <div className="chat-empty-panel">
          {renderModelSelectorCard(showDropdownSelector)}
          {showDropdownSelector && (
            <button className="btn btn-secondary btn-close-selector" onClick={() => setShowDropdownSelector(false)}>
              取消修改并返回
            </button>
          )}
        </div>
  ...
  return (
    <div className="chat-view">
      ...
      <div className="chat-floating-pill-container">
